...
# %sh
# "/Workspace/Repos/nguyen-hoang.tam@unilever.com/ds_projects/demand_sensing_yafsu_run/dev/APPLICATION_YAFSU/ray.sh"
...
display(df_input_total.agg(MAX_YW = ("YEARWEEK", max)))

# COMMAND ----------

display(df_input_total.shape[0])
...
df_input_cs_week = df_input_total.groupby(["DISTRIBUTOR", "DPNAME", "YEARWEEK", "BANNER"]).agg(CS_WEEK = ("CS_WEEK", "sum")).reset_index() #change the dataset to get nonbanded only or total
df_input_cs_week["YEARWEEK"] = df_input_cs_week["YEARWEEK"].astype(int)

display(df_input_cs_week.shape[0])
...
df_features = df_features[["DPNAME", "BANNER", "YEARWEEK", "FIRST_WEEK", "MID_WEEK", "LAST_WEEK", "HOLIDAY_WEEK"]]
# display(df_features)
...
df_input = df_input_cs_week.copy()
df_input["KEY_TS"] = df_input["DISTRIBUTOR"].astype(str) +  "|"  + df_input["DPNAME"].astype(str)
...
# exo_vars = ["FIRST_WEEK", "LAST_WEEK", "MID_WEEK", "HOLIDAY_WEEK"]
...
run_yearweek = run_year*100 + run_week_order -1
...
for target_var in target_list:
    os.makedirs(
    name=f"/dbfs/mnt/adls/DT_PREDICTEDORDER/OUTPUTS/TOTAL/{target_var}",
    exist_ok=True,
)
    for yearweek_cutoff in yw_cutoff_range:
        ...
        pkl.dump(
            output_data,
            open(f"/dbfs/mnt/adls/DT_PREDICTEDORDER/OUTPUTS/TOTAL/{target_var}/{run_on}_LE_DTSS_TOTAL_NO_PROMO_{target_var}_CUTOFF_{yearweek_cutoff}.pkl", "wb")
        )
        #2
        ...
        df_no_error_result.to_parquet(
            f"/dbfs/mnt/adls/DT_PREDICTEDORDER/OUTPUTS/TOTAL/{target_var}/{run_on}_LE_DTSS_TOTAL_NO_PROMO_{target_var}_CUTOFF_{yearweek_cutoff}.parquet", index=False
        )
