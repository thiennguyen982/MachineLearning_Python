...
for target_var in target_list:
    os.makedirs(
    name=f"/dbfs/mnt/adls/DT_PREDICTEDORDER/TESTING_OUTPUTS/NON_BANDED_NO_PROMO/{target_var}",
    exist_ok=True,
)
    for yearweek_cutoff in yw_cutoff_range:
        ...
        pkl.dump(
            output_data,
            open(f"/dbfs/mnt/adls/DT_PREDICTEDORDER/TESTING_OUTPUTS/NON_BANDED_NO_PROMO/{target_var}/{run_on}_LE_DTSS_{target_var}_CUTOFF_{yearweek_cutoff}.pkl", "wb")
        )
        #2
        ...
        df_no_error_result.to_parquet(
            f"/dbfs/mnt/adls/DT_PREDICTEDORDER/TESTING_OUTPUTS/NON_BANDED_NO_PROMO/{target_var}/{run_on}_LE_DTSS_{target_var}_CUTOFF_{yearweek_cutoff}.parquet", index=False
        )
