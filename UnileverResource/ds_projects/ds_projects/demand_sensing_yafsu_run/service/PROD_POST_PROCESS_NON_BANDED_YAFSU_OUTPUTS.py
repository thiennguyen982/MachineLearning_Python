...
df_iso_cal = pd.read_excel("/dbfs/mnt/adls/DT_PREDICTEDORDER/LE_SEC_SALES_DEPENDENT_FILES/iso_cal.xlsx",header = 0)
df_iso_cal = spark.createDataFrame(df_iso_cal)
display(df_iso_cal)
...
df_input = df_input.where("Banded == 'NO'")
...
import os
run_on = d.date.today().strftime("%Y%m%d")
...
for dayOfWeek in dayOfWeekRange:
    for YWcutOff in yw_cutoff_range:
        link_download = f"/mnt/adls/DT_PREDICTEDORDER/OUTPUTS/NON_BANDED/{dayOfWeek}/{run_on}_LE_DTSS_NONBAND_NO_PROMO_CS_WEEK_CUTOFF_{YWcutOff}.parquet"
    ...
    df_full_pd.to_csv(f"/dbfs/mnt/adls/DT_PREDICTEDORDER/POST_PROCESSED_OUTPUTS/NON_BANDED/{dayOfWeek}/{dayOfWeek}_YAFSU_DTSS_PREDS_NO_PROMO_NON_BANDED_CUTOFF_{YWcutOff}_FC_{YWcutOff + 2}.csv")
