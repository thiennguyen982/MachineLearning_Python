...
def performance_check(key, df_group, actual_col, predict_col):
    pfm = abs((df_group[actual_col].sum() - df_group[predict_col].sum()) / df_group[actual_col].sum())
    performance = {
        "Layer": key,
        "Performance": pfm,
...
df_model = df_product_tpr.copy().drop(["SEC_SALES", "SEC_BASELINE_ORG", "BANNER", "REGION"], axis=1)
...
df_model = df_model[((df_model["PCT_UPLIFT"].between(-1, 1)) & (df_model["YEARWEEK"] <= 202326)) | (df_model["YEARWEEK"] > 202326)]
# df_model["ABS_UPLIFT"] = np.where(df_model["PCT_UPLIFT"].between(-0.5, 0.5), df_model["ABS_UPLIFT"], df_model["PRI_BASELINE"] * 0.5)
...
df_evaluate["ACTUAL_PCT_UPLIFT"] = np.where(
    df_evaluate["PRI_BASELINE"] == 0,
    0,
    df_evaluate["ACTUAL_UPLIFT"] / df_evaluate["PRI_BASELINE"],
)
df_evaluate["PREDICTED_PCT_UPLIFT"] = np.where(
    df_evaluate["PRI_BASELINE"] == 0,
    0,
...
print(abs(df_agg_total["PREDICTED_UPLIFT"] / df_agg_total["ACTUAL_UPLIFT"] - 1 ).mean())
print(abs(df_agg_total["PREDICTED_TOTAL"] / df_agg_total["ACTUAL_TOTAL"] - 1 ).mean())
print(abs(df_agg_total["PRI_BASELINE"] / df_agg_total["ACTUAL_TOTAL"] - 1 ).mean())
...
# df_evaluate["BIN_UPLIFT_cut"] = pd.cut(
#     df_evaluate["ACTUAL_UPLIFT"],
#     bins=np.linspace(-1, 1, 21),
# )
# print(df_evaluate["BIN_UPLIFT_cut"].value_counts(normalize=True).sort_index())
...
df_evaluate["BIN_UPLIFT_qcut"] = pd.qcut(
    df_evaluate["ACTUAL_PCT_UPLIFT"], q=10, duplicates = "drop"
)
print(df_evaluate["BIN_UPLIFT_qcut"].value_counts(normalize = True).sort_index())

pd.DataFrame.from_dict(
        [
            performance_check(key, df_group, "ACTUAL_PCT_UPLIFT", "PREDICTED_PCT_UPLIFT")
            for key, df_group in  df_evaluate.groupby("BIN_UPLIFT_qcut")
        ]
).sort_values("Layer")
...
# df_evaluate["PFM_TOTAL"] = np.where(df_evaluate["ACTUAL_TOTAL"] == 0, 0,abs(df_evaluate["PREDICTED_TOTAL"]/df_evaluate["ACTUAL_TOTAL"] - 1))
# df_evaluate["PFM_UPLIFT"] = np.where(df_evaluate["ACTUAL_UPLIFT"] == 0, 0,abs(df_evaluate["PREDICTED_UPLIFT"]/df_evaluate["ACTUAL_UPLIFT"] - 1))
# pd.pivot_table(df_evaluate, values = "PFM_TOTAL", index = "BIN_UPLIFT_qcut", columns = "CATEGORY")
...
df_agg_total["BIN"] = pd.qcut(df_agg_total["ABS"], q = 10)
df_agg_total.groupby(["BIN"])[["PCT_NEG_UPLIFT","PCT_POS_UPLIFT"]].sum().sort_index()
...
# def tune_model(X, y):
#     pipe = Pipeline(
#         steps=[("tree", LGBMClassifier(max_depth=-1, n_jobs=-1))],
#         verbose=True,
#     )
#     param_grid = {
#         # "tree__max_depth" : [-1, 4, np.arange(8, 65, 8)],
...
##{'tree__boosting_type': 'gbdt', 'tree__learning_rate': 0.1, 'tree__n_estimators': 200, 'tree__reg_alpha': 0.1, 'tree__reg_lamda': 0}
...
display(df_model[df_model["YEARWEEK"].between(202201, 2022352)].corrwith(df_model["CLASSIFY_UPLIFT"],method = pointbiserialr).T.reset_index())
...
#     pos_ratio = df_model[(df_model["DPNAME"] == cat) & (df_model["YEARWEEK"] <= 202326) & (df_model["CLASSIFY_UPLIFT"] == 1) & (df_model["PRI_BASELINE"] != 0) & (df_model["PCT_UPLIFT"].between(-0.5, 0.5))]["PCT_UPLIFT"].mean()
#     df_evaluate["RATIO"] = np.where((df_evaluate["DPNAME"] == cat) & (df_evaluate["PREDICTED_CLASSIFY"] == 1), pos_ratio, df_evaluate["RATIO"])

#     neg_ratio = df_model[(df_model["DPNAME"] == cat) & (df_model["YEARWEEK"] <= 202326) & (df_model["CLASSIFY_UPLIFT"] == -1) & (df_model["PRI_BASELINE"] != 0) & (df_model["PCT_UPLIFT"].between(-0.5, 0.5))]["PCT_UPLIFT"].mean()
#     df_evaluate["RATIO"] = np.where((df_evaluate["DPNAME"] == cat) & (df_evaluate["PREDICTED_CLASSIFY"] == -1), neg_ratio, df_evaluate["RATIO"])

# df_evaluate["PREDICTED_UPLIFT"] = df_evaluate["PRI_BASELINE"] * df_evaluate["RATIO"]

# df_evaluate["PREDICTED_UPLIFT"] = np.where(
#     df_evaluate["PREDICTED_CLASSIFY"] == 1, df_evaluate["PRI_BASELINE"] * 0.73, 
#     df_evaluate["PRI_BASELINE"] * -0.51
# )
df_evaluate["PREDICTED_UPLIFT"] = df_evaluate["PREDICTED_CLASSIFY"] * df_evaluate["RATIO_MEDIAN_UPLIFT"] * df_evaluate["PRI_BASELINE"]
...
print(abs(df_agg_total["PREDICTED_UPLIFT"] / df_agg_total["ABS_UPLIFT"] - 1 ).mean())
print(abs(df_agg_total["PREDICTED_TOTAL"] / df_agg_total["ACTUAL_TOTAL"] - 1 ).mean())
print(abs(df_agg_total["PRI_BASELINE"] / df_agg_total["ACTUAL_TOTAL"] - 1 ).mean())
display(df_agg_total)
...
df_evaluate[["CATEGORY","DPNAME","YEARWEEK","YEAR","MONTH","WEEK_MONTH_COUNT","WEEK_OF_MONTH","BANDED_COUNT","PRI_SALES","PRI_BASELINE","ACTUAL_CLASSIFY","PREDICTED_CLASSIFY","ABS_UPLIFT","PREDICTED_UPLIFT","RATIO_MEDIAN_UPLIFT","ACTUAL_TOTAL","PREDICTED_TOTAL"]].to_csv("/dbfs/mnt/adls/NMHDAT_SNOP/df_eval_tpr.csv")

# COMMAND ----------

df_agg_total = df_evaluate.groupby("YEARWEEK")[["PRI_SALES_WITHOUT_BANDED","PRI_BASELINE","ACTUAL_UPLIFT","PREDICTED_UPLIFT"]].sum().reset_index()
df_agg_total["YEARWEEK"] = df_agg_total["YEARWEEK"].astype(str)
display(df_agg_total)
