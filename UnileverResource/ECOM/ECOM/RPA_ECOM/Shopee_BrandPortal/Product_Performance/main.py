...
def run_job(job_name, from_date, to_date):
    ...
    downloaded_file_path = download_data(from_date, to_date, PRODUCT_PERFORMANCE_TMP_DIR, PRODUCT_PERFORMANCE_FILE_PATTERN)
...
#         data_df = data_df.withColumn("Date", lit(f"{to_date.strftime('%Y-%m-%d')}")).drop("No.").toPandas()
        ...
        if job_name == 'yesterday' or 'specificed_date' in job_name:
            ...
            yesterday_df.to_parquet(
                f"/dbfs/mnt/adls/staging/ecom/BOT/Shopee/BrandPortal_ProductPerformance/shopee_product_performance_{to_date.strftime('%Y_%m_%d')}.snappy"
                ,index=False)
        
        output_path = f"/dbfs/mnt/adls/staging/ecom/BOT/Shopee/BrandPortal_DailyOfftake/product_performance/shopee_dailyofftake_{job_name}.snappy"
...
browser.get('https://brandportal.shopee.com/seller/insights/product/performance')
for job in jobs:
    retry_count = 1
    job_detail = jobs[job]
    while retry_count <= 3:
        try:
            ...
            if retry_count > 3:
                ...
                dbutils.fs.mv(f"file:/tmp/screenshot.jpg", f"dbfs:/FileStore/Shopee_ProductPerformance_screenshot{datetime.now().strftime('%Y%m%d-%H%M%S')}.jpg")
                raise e
            print(f'Try again!')
            time.sleep(60)
