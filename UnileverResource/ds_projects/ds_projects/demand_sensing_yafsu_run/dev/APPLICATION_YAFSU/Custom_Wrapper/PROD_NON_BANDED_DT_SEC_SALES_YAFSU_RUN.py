...
# display(df_input_non_banded.query("DPNAME == 'VASELINE PERFECT 10 350ML'").groupby(["DPNAME", "BANNER", "YEARWEEK"]).agg(CS_WEEK = ("CS_WEEK", "sum")).reset_index())
...
# display(df_features.agg(min_YW = ("YEARWEEK", "max")))
...
# df_input = df_input_cs_week.merge(df_features, on = ["YEARWEEK", "DPNAME", "BANNER"], how = "outer")
df_input = df_input_cs_week.copy()   # no exo vars added at this point
...
for target_var in target_list:
    os.makedirs(
    name=f"/dbfs/mnt/adls/DT_PREDICTEDORDER/OUTPUTS/NON_BANDED/{target_var}",
    exist_ok=True,
)
    for yearweek_cutoff in yw_cutoff_range:
        ...
        pkl.dump(
            output_data,
            open(f"/dbfs/mnt/adls/DT_PREDICTEDORDER/OUTPUTS/NON_BANDED/{target_var}/{run_on}_LE_DTSS_NONBAND_NO_PROMO_{target_var}_CUTOFF_{yearweek_cutoff}.pkl", "wb")
        )
        #2
        ...
        df_no_error_result.to_parquet(
            f"/dbfs/mnt/adls/DT_PREDICTEDORDER/OUTPUTS/NON_BANDED/{target_var}/{run_on}_LE_DTSS_NONBAND_NO_PROMO_{target_var}_CUTOFF_{yearweek_cutoff}.parquet", index=False
        )
