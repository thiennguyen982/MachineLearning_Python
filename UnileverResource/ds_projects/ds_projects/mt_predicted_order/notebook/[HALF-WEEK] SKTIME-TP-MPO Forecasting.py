...
from mt_predicted_order.common.sklearn import fa_avg_scorer, fa_sumup_scorer, fa_avg_score, fa_sumup_score, TimeSeriesFE, fb_score, fb_scorer
...
# MAGIC select
# MAGIC *,
# MAGIC dense_rank() over (order by yearweek, half_week) as timepoints
# MAGIC from calendar
# MAGIC ),
...
# MAGIC pi.dp_name,
# MAGIC yearweek,
# MAGIC cal.timepoints as ds,
# MAGIC concat_ws("|", df.banner, df.region, pi.dp_name) as unique_id,
# MAGIC
# MAGIC -- sale
...
# MAGIC final.*
# MAGIC from final
# MAGIC inner join filter_zero_demand on final.unique_id = filter_zero_demand.unique_id 

# COMMAND ----------

# _sqldf.cache().createOrReplaceTempView("data_mpo_final")
_sqldf.write.mode("overwrite").option("overwriteSchema", "true").saveAsTable("mt_feature_store.data_tp_halfweek")
...
train, test = list(SlidingWindowSplitter(fh=[1, 2, 3, 4], window_length = 192, step_length=1).split(data.loc[:, numerical_columns + lag_columns + categorical_columns]))[0]
...
sktime_regressor = LGBMRegressor(n_jobs = -1,
                  categorical_features = [idx for idx, col in enumerate(categorical_columns)],
  verbose = -1)
sktime_regressor = LSTMFCNRegressor()
target = TransformedTargetForecaster(steps=[
  ...
  # ("imputer", Imputer(method = "backfill")),
  ("forecaster", make_reduction(sktime_regressor,
                                window_length = 4,
...
for idx, (train, test) in enumerate(SlidingWindowSplitter(fh=[1, 2, 3, 4], window_length = 192, step_length=1).split(data[["est_demand"]])):
  ...
  print("Start fitting with sktime pipeline")
  print(f"X_train: \n{X_train.head()}")
  sktime_pipe.fit(y = y_train, X = X_train)
  y_pred = sktime_pipe.predict(fh = [1, 2, 3, 4], X = X_test)
  fa_avg = fa_avg_score(y_test, y_pred)
  print(f"[SKTIME] FA_avg: {fa_avg}")

  # print("Start fiting with sklearn pipeline ...")
...
df_result = pd.concat(df_results).join(data_inverse.loc[:, ["banner", "region", "dp_name", "category", "yearweek", "half_week"]])
...
fa_sumup = fa_sumup_score(df_result[label], df_result[prediction])
fb = fb_score(df_result[label], df_result[prediction])
display(pd.DataFrame({"fa_avg": [fa_avg], "fa_sumup": [fa_sumup], "fb": [fb]}))

df_metric = []
# breaking down dimension
for group in [["horizon"], ["banner"], ["category"], ["region"], ["banner", "region"]]:
    fa_avg = df_result.groupby(group).apply(
    lambda x: fa_avg_score(x[label], x[prediction])
    ).reset_index(name=f'fa_avg')
    fa_avg["group"] = '_'.join(group)
    fa_sumup = df_result.groupby(group).apply(
    lambda x: fa_sumup_score(x[label], x[prediction])
    ).reset_index(name=f'fa_sumup')

    fb = df_result.groupby(group).apply(
    lambda x: fb_score(x[label], x[prediction])
    ).reset_index(name=f'fb')
    display(pd.concat([
        fa_avg.loc[:, ["group"] + group + ["fa_avg"]],
        fa_sumup.loc[:, ["fa_sumup"]],
        fb.loc[:, ["fb"]],
    ], axis = 1).style.background_gradient(axis=0))
...
spark.createDataFrame(df_result).write.mode("overwrite").option("overwriteSchema", "true").saveAsTable("mt_feature_store.mpo_halfweek_v0_0_2")
